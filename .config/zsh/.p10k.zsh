...
() {
  ...
  # The list of segments shown on the left. Fill it with the most important
  # segments.
  ...
  # The list of segments shown on the right. Fill it with less important
  # segments. Right prompt on the last prompt line (where you are typing your
  # commands) gets automatically hidden when the input line reaches it. Right
  # prompt above the last prompt line gets hidden if it would overlap with left
  # prompt.
  ...
  # When set to false, icons go after content. If empty or not set, icons go
  # before content in the left prompt and after content in the right prompt.
  #
  ...
  # Connect left prompt lines with these symbols. You'll probably want to use
  # the same color as POWERLEVEL9K_MULTILINE_FIRST_PROMPT_GAP_FOREGROUND below.
  ...
  # Filler between left and right prompt on the first prompt line. You can set
  # it to ' ', '·' or '─'. The last two make it easier to see the alignment
  # between left and right prompt and to separate prompt from command output.
  # You might want to set POWERLEVEL9K_PROMPT_ADD_NEWLINE=false for more
  # compact prompt if using this option.
  ...
  # To remove a separator between two segments, add "_joined" to the second
  # segment name. For example: POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(os_icon
  # context_joined)
  ...
  # doesn't fit or when other parameters demand it (see
  # POWERLEVEL9K_DIR_MIN_COMMAND_COLUMNS and
  # POWERLEVEL9K_DIR_MIN_COMMAND_COLUMNS_PCT below). If set to `0`, directory
  ...
  # opening a directory in the file manager simply by clicking the link. Can
  # also be handy when the directory is shortened, as it allows you to see the
  # full directory that was used in previous commands.
  typeset -g POWERLEVEL9K_DIR_HYPERLINK=false

  # Enable special styling for non-writable and non-existent directories. See
  # POWERLEVEL9K_LOCK_ICON and POWERLEVEL9K_DIR_CLASSES below.
  typeset -g POWERLEVEL9K_DIR_SHOW_WRITABLE=v3

  # POWERLEVEL9K_DIR_CLASSES allows you to specify custom icons and colors for
  # different directories. It must be an array with 3 * N elements. Each
  # triplet consists of:
  #
  #   1. A pattern against which the current directory ($PWD) is matched.
  #   Matching is done with extended_glob option enabled. 2. Directory class
  #   for the purpose of styling. 3. An empty string.
  #
  # Triplets are tried in order. The first triplet whose pattern matches $PWD
  # wins.
  #
  # If POWERLEVEL9K_DIR_SHOW_WRITABLE is set to v3, non-writable and
  # non-existent directories acquire class suffix _NOT_WRITABLE and
  # NON_EXISTENT respectively.
  #
  ...
  #     '*'            DEFAULT  '')
  #
  # Whenever the current directory is ~/work or a subdirectory of ~/work, it
  # gets styled with one of the following classes depending on its writability
  # and existence: WORK, WORK_NOT_WRITABLE or WORK_NON_EXISTENT.
  #
  # Simply assigning classes to directories doesn't have any visible effects.
  # It merely gives you an option to define custom colors and icons for
  # different directory classes.
  #
  #   # Styling for WORK.
  ...
  #
  # If a styling parameter isn't explicitly defined for some class, it falls
  # back to the classless parameter. For example, if
  # POWERLEVEL9K_DIR_WORK_NOT_WRITABLE_FOREGROUND is not set, it falls back to
  ...
  # Branch icon. Set this parameter to '\UE0A0 ' for the popular Powerline
  # branch icon.
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      # Otherwise show the first 12 … the last 12. Tip: To always show local
      # branch name in full without truncation, delete the next line.
    ...
    if [[ -n $VCS_STATUS_TAG
      ...
      # If tag name is at most 32 characters long, show it in full. Otherwise
      # show the first 12 … the last 12. Tip: To always show tag name in full
      # without truncation, delete the next line.
      (( $#tag > 32 )) && tag[13,-13]="…"  # <-- this line
      res+="${meta}#${clean}${tag//\%/%%}"
    fi

    # Display the current Git commit if there is no branch and no tag. Tip: To
    # always display the current Git commit, delete the next line.
    ...
    if [[ $VCS_STATUS_COMMIT_SUMMARY ==
      (|*[^[:alnum:]])(wip|WIP)(|[^[:alnum:]]*) ]]; then res+=" ${modified}wip"
    fi
    ...
    if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
      ...
      # Tip: Uncomment the next line to display '=' if up to date with the
      # remote. res+=" ${clean}="
    fi
    ...
    # ?42 if have untracked files. It's really a question mark, your font isn't
    # broken. See POWERLEVEL9K_VCS_UNTRACKED_ICON above if you want to use a
    # different icon. Remove the next line if you don't want to see untracked
    # files at all.
  ...
  # seeing performance sagging, try setting
  # POWERLEVEL9K_VCS_MAX_INDEX_SIZE_DIRTY to a number lower than the output of
  # `git ls-files | wc -l`. Alternatively, add `bash.showDirtyState = false` to
  # the repository's config: `git config bash.showDirtyState false`.
  ...
  # Show status of repositories of these types. You can add svn and/or hg if
  # you are using them. If you do, your prompt may become slow even when your
  # current directory isn't in an svn or hg repository.
  ...
  #              files.
  #   - quiet:   Enable instant prompt and don't print warnings when detecting
  #              console output during zsh initialization. Choose this if
  #              you've read and understood
  #              https://github.com/romkatv/powerlevel10k/blob/master/README.md#instant-prompt.
  #   - verbose: Enable instant prompt and print a warning when detecting
  #              console output during zsh initialization. Choose this if
  #              you've never tried instant prompt, haven't seen the warning,
  #              or if you are unsure what this all means.
  typeset -g POWERLEVEL9K_INSTANT_PROMPT=verbose

  # Hot reload allows you to change POWERLEVEL9K options after Powerlevel10k
  # has been initialized. For example, you can type POWERLEVEL9K_BACKGROUND=red
  # and see your prompt turn red. Hot reload can slow down prompt by 1-2
  # milliseconds, so it's better to keep it turned off unless you really need
  # it.
...
typeset -g POWERLEVEL9K_CONFIG_FILE="$ZDOTDIR/.pk10k.zsh.new"
